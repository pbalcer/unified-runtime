...
struct ur_completion_batch {
  ...
  enum state {
    ...
    SEALED,       // checkComplete() || forceWait() -> COMPLETED
  ...
  // Synchronously waits for all events in the batch to complete.
  ur_result_t forceWait();

  // Resets a complete batch back to an empty state. Cleanups internal state
  // but keeps allocated memory for reuse.
  ur_result_t reset();
...
struct ur_completion_batches {
  ...
  // This function might also check for batch completions, and prune the
...
struct ur_command_list_info_t {
  ur_command_list_info_t(ze_fence_handle_t ZeFence, bool ZeFenceInUse,
      ...
      : ZeFence(ZeFence), ZeFenceInUse(ZeFenceInUse), IsClosed(IsClosed),
        ZeQueue(ZeQueue), ZeQueueDesc(ZeQueueDesc), CanReuse(CanReuse) {
