...
static class {
public:
  ur_adapter_handle_t get() {
    std::shared_lock lock(rwlock);
    if (adapter == nullptr) {
      adapter = std::make_unique<ur_adapter_handle_t_>();
    }
    return adapter.get();
  }
  void teardown() {
    ...
    auto ptr = adapter.release();
    delete ptr;
  }

private:
  std::shared_mutex rwlock;
  std::unique_ptr<ur_adapter_handle_t_> adapter;
} AdapterWrapper;

ur_adapter_handle_t getAdapter() { return AdapterWrapper.get(); }
